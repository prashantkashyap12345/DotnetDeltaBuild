# Build only projects with unpushed Git changes (committed/unpushed + staged + unstaged)
dnbgc() {
    ...
    # Check which projects contain these changes
    while IFS= read -r project; do
        local project_dir=$(dirname "$project")
        local has_changes=false

        while IFS= read -r changed_file; do
            local abs_changed_file="$repo_root/$changed_file"
            if [[ "$abs_changed_file" == "$project_dir"/* ]]; then
                has_changes=true
                break
            fi
        done <<< "$changed_files"
    ...
    while IFS= read -r changed_file; do
        if [[ "$changed_file" == *.csproj ]]; then
            ...
            if [ -f "$project" ] && [ -z "${project_map[$project]}" ]; then
                projects_to_build+=("$project")
            fi
        fi
    done <<< "$changed_files"
...
dnbgcq() {
    ...
    while IFS= read -r project; do
        ...
        while IFS= read -r changed_file; do
            local abs_changed_file="$repo_root/$changed_file"
            if [[ "$abs_changed_file" == "$project_dir"/* ]] || [[ "$changed_file" == *.csproj ]]; then
                has_changes=true
                break
            fi
        done <<< "$changed_files"
...
dnbgc-dry() {
    ...
    # Find all .csproj files (more reliable method)
    local all_projects=$(find "$(git rev-parse --show-toplevel)" -name "*.csproj" -type f)
    if [ -z "$all_projects" ]; then
        echo "  No .csproj files found!"
        return 1
    fi

    echo "  Found these .csproj files:"
    echo "$all_projects" | sed 's/^/    /'
    ...
    while IFS= read -r project; do
        ...
        while IFS= read -r changed_file; do
            if [[ "$(git rev-parse --show-toplevel)/$changed_file" == "$project_dir"/* ]]; then
                has_changes=true
                break
            fi
        done <<< "$changed_files"
